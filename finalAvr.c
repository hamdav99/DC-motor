...
volatile int8_t i = 0; // determines i of array
...
int init_INTs(void)
{
	...
	PCMSK1 |= (1<<PCINT11)|(1<<PCINT10);   // PCINT8 and PCINT9  enabled (encoder)
...
ISR(TIMER2_COMPA_vect) {
	cli();
	if(PIBoolean > 2){ // bit of duct tape to control how often code runs in an easier way. 
...
ISR(TIMER1_COMPA_vect) {
	...
	cli();
	if (timeBoolean == 1) { // bit of duct tape to control how often code runs in an easier way. 
...
ISR(USART_RX_vect)
{
	...
	//waits for three (or less) chars to transform into an int
		...
		targetValue =  (num1 - '0') * 100 + (num2 - '0') * 10 + ((num3-4) - '0'); // -'0' casts int to chars
...
int PI_control(uint16_t setpoint, uint16_t feedback) {
	...
	if(error_sum > 4<<NUM_FRACTION_BITS) { // Avoid saturation of the integral part, multiplication with 2 and power to match format of other values
		error_sum =  4*2^NUM_FRACTION_BITS;
		PORTB = PORTB | (1<<PB6);
		PORTB = PORTB & ~(1<<PB7);
		} 
	else if (error_sum < -4<<NUM_FRACTION_BITS) {
		PORTB = PORTB & ~(1<<PB6);
		PORTB = PORTB | (1<<PB7);
		error_sum = - 4*2^NUM_FRACTION_BITS;
	}
